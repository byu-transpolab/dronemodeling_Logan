...
#load a model
if New_model:
    model= YOLO("yolov8s.yaml") #New model using v8 nano version
...
results = model.train(              #Determined by new_model
                     data="training/ultralytics_model/config.yaml"  #Configuation file with training and validation images desginated, as well as the classes
                    ,epochs =60
                    ,device=[-1,-1])#"mps")                             #How many times the training will run. 
