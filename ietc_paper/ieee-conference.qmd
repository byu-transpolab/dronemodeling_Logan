...
Previous studies have shown that these models suffer from a slight over-prediction bias in predicted traffic [@erhardt2020traffic].
...
Standard practice has been to employ model transferability, where a model developed in one region is applied to another.
This method works when the two regions have similar characteristics; however, is limited otherwise [@wilmot2001transferability; @bowman2017testing].
...
Using the latitude and longitude of the official city boundaries, all of the building footprints in the city were extracted from the Microsoft Building Footprints dataset. 
...
Because the majority of the buildings were in the smallest cluster, we oversampled 200 buildings from clusters that included only larger buildings. 

## Model Training
![YOLO prediction output example](images_figures/logan_val_batch1_pred.jpg){#fig-YOLO-example width=100%}


Ultralytics' "You Only Look Once", or YOLO model, is an open-source object detection model that has become very popular within the research community due to its accuracy and ease of use [@Jocher_Ultralytics_YOLO_2023].
...
Despite this, building types such as schools and church buildings were still underrepresented and appeared only a few times, due to their relatively low number even within larger buildings. 

The YOLO model is trained on the training images over hundreds of iterations,
...
The confusion matrix shows that the model accurately predicted residential buildings 99% of the time, and all commercial and apartment buildings.
However, it did underperform with churches (67%) due to the low number of buildings to train on. 
...
Given the results of the validations, we would conclude that developing a process for model creation would be more useful and viable than creating a universal model to be used in a region. 
It appears that building styles and types are unique enough between cities that a single unified model, at least one mostly dependent on identifying building roofs, is not feasible. 
...
The rise of commercially available and affordable aerial drones has improved the ability for individuals and small organizations to capture high-quality aerial imagery, with impacts felt across many fields [@colomina_unmanned_2014; @watts_unmanned_2012].
...
While the Microsoft Building Footprints dataset is expansive, it is not exhaustive. 
...
Creating our own process to extract footprints over a small area could improve the accuracy of our footprints and help not just in finding buildings but also in training other models. 
...
There are potential ethical problems with this data collection, with drone-collected imagery challenging the boundary between public and private information [@hall_use_2021].
More research is needed to address these potential privacy concerns  

The analysis of the classification of buildings near each other is another variable that could be useful in a general model. 
Similar building types are often built next to each other.
...
Currently, this research is susceptible to many of the same limitations as transportation models. 
...
The use of commercial drones could solve these problems, but they also have challenges of their own.
